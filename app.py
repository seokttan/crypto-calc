...
# [보안] 비밀번호는 Streamlit Cloud의 Settings -> Secrets에 ADMIN_PASSWORD로 저장하세요.
try:
...
with col_edit:
    with st.expander("⚙️ 내 자산 리스트 편집", expanded=False):
        # [수정] 쿠키 저장 없이 실시간 비밀번호 확인
        input_pw = st.text_input("관리자 비밀번호 입력", type="password", help="편집을 위해 비밀번호를 입력하세요.")
        
        if VALID_PASSWORD and input_pw == VALID_PASSWORD:
            st.success("인증됨: 자산을 추가하거나 삭제할 수 있습니다.")
            add_col, del_col = st.columns(2)
            with add_col:
                new_coin = st.text_input("추가할 코인", key="input_new").upper().strip()
                if st.button("추가", use_container_width=True):
                    if new_coin and get_lbank_prices([new_coin]).get(new_coin, 0) > 0:
            ...
            st.info("비밀번호를 입력하면 편집 도구가 나타납니다.")
...
while True:
    with result_area.container():
        ...
        data = [
            ["🇰🇷 KRW (현금)", "₩ 1.00", f"₩ {format_num(base_usdt * usd_to_krw, 0)}"],
