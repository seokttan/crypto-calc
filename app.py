...
# --- [4] 중간: 실시간 시세 영역 (깜빡임 방지 Fragment) ---
@st.fragment(run_every=2)  # 2초마다 이 함수 안의 내용만 '조용히' 업데이트
def show_realtime_table(base_asset, input_val):
    ...
    st.caption(f"최종 업데이트: {kor_now.strftime('%H:%M:%S')} (2초 간격 자동 갱신)")
...
# 시세 함수 실행
show_realtime_table(base_asset, input_val)
...
with st.expander("⚙️ 내 자산 리스트 편집"):
    ...
    with add_col:
        new_coin = st.text_input("추가할 심볼", key="add_input").upper().strip()
        if st.button("목록에 추가"):
    ...
    with del_col:
        if st.session_state.target_coins:
            del_target = st.selectbox("삭제할 코인 선택", st.session_state.target_coins, key="del_select")
